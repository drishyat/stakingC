...
contract StakePool is Pausable {
  ...
  /** @dev contract constructor
    ...
    */
  constructor(address payable _stakeContract) public {
    owner = msg.sender; 
  ...
  /** @dev set staking contract address
    ...
    */
  function setStakeContract(address payable _stakeContract) external onlyOwner {
    require(_stakeContract != address(0));
  ...
  function stake() external onlyOwner {
    ...
    payable(address(sc)).transfer(toStake);
   ...
   */
  function getState() external view returns ( uint[] memory) {
